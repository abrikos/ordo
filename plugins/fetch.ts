...
export default defineNuxtPlugin((_nuxtApp) => {
    ...
    function onError(e: any) {
        ...
        snackbar.add({action: undefined, dismissible: undefined, duration: undefined, role:undefined, title: '', type: 'error', text: e.request + ' ' + e.status + ':' + (e.response._data.message || e.response._data)})
    }
    ...
    async function refresh() {
        ...
        if (expire) {
            ...
            if (diffMinutes < 10) {
                const data = await $fetch('/api/v1/user/refresh_token', {headers: getHeaders()}).catch(onError) as {token: string, expire: string}
                if(data) {
